...
def configure_logging():
    ...
    log_handler = RotatingFileHandler(log_filename, maxBytes=5*1024*1024, backupCount=2, encoding=None, delay=0)
...
def pytest_terminal_summary(terminalreporter, exitstatus, config):
    ...
    for key in summary:
        if key in ['passed', 'failed', 'error', 'skipped', 'deselected', 'xfailed', 'xpassed']:
...
def pytest_runtest_logreport(report):
    if report.failed:
        logger.error(f"Test {report.nodeid} failed with the following error: {report.longrepr}")
